...
|-------|---------------|-------|-------|
| [Tên asset 1 — vd: Bookmark URL] | [Critical/High/Medium/Low] | [Team/Module] | [Lý do classification — vd: chứa private URL người dùng] |
| [Tên asset 2] | ... | ... | ... |
```
...
| **I**nformation Disclosure | Exposing data | Can data leak via error messages, logs, side channels, IDOR? | `ProblemDetails` (no stack trace in prod), Serilog destructure-redact, `[Authorize]` + ownership predicate |
...
**Component**: [Component bị ảnh hưởng — vd: `BookmarkController.Create`]
...
### Acceptance Criteria
- [ ] [Tên security test cụ thể + plan task ID — vd: `BookmarkTests.RejectsForeignUserId` / Task 3.4]
...
**Surface chọn:** [Tên surface — vd: URL-fetching for Bookmark title preview]
**Lý do chọn:** [Vì sao đây là surface rủi ro cao nhất trong feature này]

| # | Control | Threat sub-vector closed | Test in plan task | Source |
|---|---------|--------------------------|-------------------|--------|
| 1 | [Control 1 — vd: URL scheme allowlist] | [Blocks `file://`, `gopher://`] | [Task X.Y / `TitleFetcherTests.RejectsNonHttpScheme`] | [ADR-NNN] |
| 2 | [Control 2 — vd: DNS rebinding mitigation] | [Connect to resolved IP, not hostname] | [Task X.Y / test name] | [ADR-NNN] |
| 3 | [Control 3 — vd: Block cloud metadata IPs] | [Blocks 169.254.169.254, 100.100.100.200] | [Task X.Y / test name] | **[NEW]** |
| ... | ... | ... | ... | [ADR-NNN hoặc **[NEW]**] |
```
