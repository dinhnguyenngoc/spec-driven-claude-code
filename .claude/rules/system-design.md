...
await _eventPublisher.PublishAsync(
    ...
    message: new OrderCreatedEvent(order.Id, order.UserId, order.Total, DateTime.UtcNow));

// Other services subscribe independently via Kafka consumers
```
...
### Retry Strategy
```csharp
// ✅ Exponential backoff with jitter — use Polly
...
await retryPolicy.ExecuteAsync(async () => await CallExternalApiAsync());
```
