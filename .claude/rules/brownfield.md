...
- [ ] The area being modified produces/consumes a message or shares a cache structure → its schema/key definition exists in the repo (serialized class, exported `.avsc`/`.proto`, key-pattern doc — indexed by `CODEBASE_MAP.md` §Messaging/Cache inventories) **BEFORE** modifying; characterization covers the observable effect through a TestContainers-backed broker/cache (never the real one), and the change does not break the schema/key contract other systems rely on (backward-compat by default)
...
- [ ] The change alters the schema or a DB-resident object (column/table/proc/trigger/view) → (i) it is authored as a **versioned migration script in the repo** — never edited straight into the database (`database.md` §DB-resident objects are source code); (ii) **`db/schema-snapshot/` is refreshed in the SAME change-set** — re-export from the `/test` TestContainer once the migration has been applied there (that container IS the target state, so no real database is touched) and commit it, otherwise the baseline evidence silently rots from the first change onward; (iii) the system is live → **expand-contract** (`database.md`), not an in-place rename/drop
