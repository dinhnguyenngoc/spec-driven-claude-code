...
- [ ] **Scenario coverage** — every `@US-XXX-Snn` from the spec maps to a **wired** path **and** to a test that asserts that scenario's observable *Then* (not just an isolated unit / endpoint).
...
2. **Five-Axis Scores** table — numerical 1-5 per axis with one-line justification:
   ```markdown
   ...
   | 5 | Performance | 5 | … |
   ```
   If `REQUEST CHANGES → APPROVED` after a fix, re-score and show the updated number.
...
6. **Compliance Check** table — every `.claude/rules/*.md` mapped to PASS / WARNING / FAIL with a one-line note. This frees `/scan` from re-checking rule compliance:
   ```markdown
   | Rule | Status | Notes |
   |------|--------|-------|
   | clean-code.md | PASS | … |
   | code-style.md | WARNING | `.editorconfig` over-broad rule (W-5) |
   | security.md | WARNING | RC-5.2 unimplemented (W-3) |
   | … | … | … |
   ```
